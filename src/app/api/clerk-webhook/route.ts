...
// import { NextResponse } from 'next/server'; // Removed as it's not directly used for response creation here
import prisma from "@/lib/prisma";

export async function POST(req: Request) {
  // 1. Get the headers
  const headerPayload = await headers(); // Await headers() as it returns a Promise
  ...
  if (!WEBHOOK_SECRET) {
    throw new Error(
      "You must provide a CLERK_WEBHOOK_SECRET environment variable."
    );
  }
